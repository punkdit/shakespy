...
def possy(horses):
    ...
    #strengths (secret)- 2
    #speed 50-80, strength 70-90 around about
    ...
    oddslist = sorted(sortpossy.keys())
    for oddies in oddslist:
        u.possy.append(sortpossy[oddies]) #append the horses
...
def raceroutine(segments):
    ...
    for segment in range(segments):
        ...
        if segger == 'first': possy(u.lanes.values())
        else: possy(u.possy)
...
def sleep():
    ...
    sh(2); print('\n sleep time..\n'); sh(1)
    for y in range(d(3, 5)):
        for i in range(d(2, 6)): print('.', end="")
        print('...zzZ..'); sh(d(0, 2))
        print('\n' * d(0, 1))
...
def odds(horse):
    ...
    odds = round((float(u.horses_per) * horse.rank /
                        (u.horses_per - 2) / 10), 1)
    horse.odds = odds
    horse.oddstring = str(format(odds, '0.2f')) + ' to 1'
...
def guide():
    ...
    while lane - 1 not in range(u.horses_per):
        if lane == 0: break
        try: lane = int(raw_input("type 0 to close guide\n"))
        except: continue
...
def bet(lane):
    ...
    while True:
        ...
        if len(u.ticket) < u.bets and u.money >= 1:
            ...
            a = raw_input('  Enter 1 to bet again.  ')
            if a == '1':
...
def broadcast():
    ...
    print('        The alarm goes off.',end=""); sh(2)
    ...
    tis = raw_input("Very interesting, wouldn't you say?\n\n")
    print('\nindeed..', tis)
...
def main(game='load'):
    ...
    you = raw_input('Enter your name: ').strip()
    u = U(you)
    ...
    while u.races not in range(6) or u.races == 0:
        try: u.races = int(raw_input(u.races))
        except: continue
    ...
    while u.horses_per not in range(11) or u.horses_per < 6:
        try: u.horses_per = int(raw_input(u.horses_per))
        except: continue
